...
# 动作序列时间参数（单位：秒）
STOP_BEFORE_FORWARD = 0.5    # 抓取前停止等待时间
FORWARD_TO_GRAB = 0.8        # 抓取前前进时间
STOP_AFTER_GRAB = 0.5        # 抓取后停止等待时间
...
# 任务2相关参数
TASK2_FIELD_COLORS = ['blue', 'red']  # 任务2中搜索的场地颜色
FIELD_COLOR_PRIORITY = ['blue', 'red']  # 场地颜色优先级
FIELD_DETECTION_CONFIDENCE = 0.7  # 场地检测置信度阈值
FIELD_APPROACH_DISTANCE = 0.45      # 接近场地的目标距离（米）

# 配置参数部分添加或修改以下参数
FIELD_SEARCH_ROTATION_TIME = 0.3  # 每次旋转的时间
FIELD_SEARCH_PAUSE_TIME = 0.2    # 旋转后的暂停时间
FIELD_MIN_AREA = 1000             # 最小场地面积
FIELD_ASPECT_RATIO_MIN = 1.2      # 最小长宽比
...
class JoystickController:
    def __init__(self, serial_port='/dev/ttyUSB0', baudrate=115200, timeout=1):
        ...
        self.lock = threading.Lock()  # Protect access to auto_mode
...
class YOLOProcessor:
    ...
    def detect_field_regions(self, frame, depth_image):
        ...
        # 调整HSV阈值，使其更容易检测到场地
        LOWER_RED1 = np.array([0, 70, 50])     # 降低饱和度和亮度要求
        UPPER_RED1 = np.array([10, 255, 255])
        LOWER_RED2 = np.array([160, 70, 50])   # 降低饱和度和亮度要求
        UPPER_RED2 = np.array([179, 255, 255])
        LOWER_BLUE = np.array([100, 70, 50])   # 降低饱和度和亮度要求
        UPPER_BLUE = np.array([140, 255, 255])
        LOWER_PURPLE = np.array([130, 30, 30])
        UPPER_PURPLE = np.array([155, 255, 255])
        ...
        # 使用更大的核进行形态学操作
        kernel = np.ones((11, 11), np.uint8)  # 增加核的大小
        ...
        if not purple_strips:
            ...
            self.latest_field_center = None  # Reset field center
            return False

        # 处理红色场地
        red_found = self.process_field(mask_red, "Red Field", (0, 0, 255), purple_strips, mask_purple, frame, depth_image)

        # 如果未检测到红色场地，则处理蓝色场地
        if not red_found:
            blue_found = self.process_field(mask_blue, "Blue Field", (255, 0, 0), purple_strips, mask_purple, frame, depth_image)
            if not blue_found:
                self.logger.warning("未检测到红色或蓝色场地。")
                self.latest_field_distance = None
                self.latest_field_center = None  # Reset field center
                return False
            else:
                self.logger.info("检测到蓝色场地。")
        else:
            self.logger.info("检测到红色场地。")
    ...
    def process_field(self, mask, field_name, field_color, purple_strips, mask_purple, frame, depth_image):
        ...
        for contour in contours:
            ...
            self.logger.info(f"检测到{field_name}，距离={distance}米，中心点=({center_x}, {center_y})")
    ...
    def detect_and_process_fields(self, color_frame, depth_image):
        ...
        if field_detected:
            ...
            self.logger.info(f"场地中心偏移: X={offset_x}, Y={offset_y}, 距离={distance:.2f}米")

            if not self.in_grab_action and distance < APPROACH_THRESHOLD:
                self.logger.info("距离场地25厘米，执行抓取动作序列。")
                ...
                self.joystick_controller.send_command("5")  # 停止
                time.sleep(STOP_AFTER_GRAB)
                self.logger.info("完成抓取动作序列，返回任务一模式。")
                self.task_state = 'SEARCH_BALL'  # 切换回任务一模式
                self.in_grab_action = False
            elif self.in_grab_action and distance > EXIT_THRESHOLD:
                self.logger.info("距离场地大于退出阈值，停止抓取动作。")
                self.in_grab_action = False
                self.joystick_controller.send_command("5")  # 停止
            else:
